			...
			<template v-if="title">
				<div class="flex flex-row items-center" ref="modalTitleRef" style="width: 100%; cursor: move">
					<template v-if="!hideBack">
						<ArrowLeftOutlined class="mr-2 cursor-pointer hv-active" @click="back(true)"/>
						<ArrowRightOutlined class="mr-4 cursor-pointer hv-active" @click="back(false)"/>
					</template>
...
watch(visible, (val) => {
	if (!val) {
		useConst().value.hasPopup[props.name] = false
		emit('clear')
		emit('close')
	}
});
...
function onOpen (popupName: string) {
	...
	visible.value = true;
	emit('open')
	useConst().value.hasPopup[props.name] = true
}
...
function back(str: boolean) {
	if (str && window.location.pathname.includes('/tin-tuc/')) {
		window.history.go(-2);
		return
	}
	_historyURL(str);
